...
PolarSynchronizer.prototype.synchronize = function(cb, progressCb) {
	...
	async.waterfall([
		...
		function(lastTimeStamp, cb) {
			...
			if (me.customEndTimeStamp) {
				endTimeStamp = me.customEndTimeStamp;
			}
					...
					if(!error) {
						...
						if(!result.correctMimeType) {
							...
							if(!result.downloaded) {
								existingFiles.push(trainingUrl);
							} else {
		...
		cb(error, nrFilesDownloaded, couldNotDownload, falseMimeTypes, successfullDownloads, existingFiles);
	});
};

PolarSynchronizer.prototype.processTraining = function(training, cb) {
	var me = this;
	var exportUrl = training.url + "/export/tcx/true";
...
PolarSynchronizer.prototype.getLastTimeStamp = function(cb) {
	if (this.customStartTimeStamp) {
		cb (null, this.customStartTimeStamp);		
	}
	else {
		var me = this;
...
PolarSynchronizer.prototype.setCustomStartTimeStamp = function (customTimeStamp) {
	this.customStartTimeStamp = moment(customTimeStamp, "DD.MM.YYYY").add(1, 'd');
}

PolarSynchronizer.prototype.setCustomEndTimeStamp = function(customTimeStamp) {
	this.customEndTimeStamp = moment(customTimeStamp, "DD.MM.YYYY");
};
