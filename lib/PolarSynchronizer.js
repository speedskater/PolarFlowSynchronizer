...
PolarSynchronizer.prototype.synchronize = function(cb, progressCb) {
	...
	async.waterfall([
				...
				this.processTraining(training, function(error, result) {
					if(!error) {
						...
						if(!result.correctMimeType) {
							falseMimeTypes.push(flowBaseUrl + training.url.toLowerCase());
						}
					} else {
						++failedFiles;
						couldNotDownload.push(flowBaseUrl + training.url.toLowerCase());
					}
					if(!!progressCb) {
		...
		cb(error, nrFilesDownloaded, couldNotDownload, falseMimeTypes);
	});
};
